...
export async function up(queryInterface, config, appLog, sequelizeDb) {
  ...
  if (dialect === 'postgres' || dialect === 'postgresql') {
    try {
      const query = `ALTER TABLE ${s}"service_tokens" ALTER COLUMN "id" TYPE VARCHAR(255);`;
    ...
    try {
      ...
      // add id column
      await queryInterface.addColumn(migrationUtils.tableRef('service_tokens', schema), 'id', {
    ...
    try {
      ...
      await queryInterface.changeColumn(migrationUtils.tableRef('service_tokens', schema), 'id', {
        type: Sequelize.STRING,
        defaultValue: Sequelize.UUIDV4,
      });
    } catch (error) {
