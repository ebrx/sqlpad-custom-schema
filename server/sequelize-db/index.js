...
class SequelizeDb {
  constructor(config) {
    ...
    if (config.get('backendDatabaseUri')) {
      ...
      // For PostgreSQL, support search_path/schema via URI query parameter
      // e.g. postgresql://user:pass@host:5432/db?search_path=myschema
      if (uri.startsWith('postgres')) {
        try {
          ...
          if (searchPath) {
            ...
            parsedUrl.searchParams.delete('search_path');
            parsedUrl.searchParams.delete('schema');
            sequelize = new Sequelize(parsedUrl.toString(), sequelizeOptions);
          }
        } catch (e) {
          appLog.warn('Failed to parse backendDatabaseUri for schema extraction: ' + e.message);
        }
      }
